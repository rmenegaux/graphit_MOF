        ...
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                wandb.log({'train/_loss': epoch_train_loss,
                          ...
                          'train/_avg_prec': epoch_train_acc,
                          'test/_avg_prec': epoch_test_acc,
...
def main():    
    ...
    # wandb
    wandb.init(project="moltox21-project", entity="mselosse")
    wandb.config = config
    ...
    dataset = PygGraphPropPredDataset(name='ogbg-moltox21', root='/scratch/prospero/mselosse/GraphiT-edges/dataset/moltox21') # /scratch/prospero/mselosse/datasets
    
    
    split_idx = dataset.get_idx_split()
    moltox21_dataset_train = dataset[split_idx['train']]
    moltox21_dataset_val = dataset[split_idx['valid']]
    moltox21_dataset_test = dataset[split_idx['test']]


    # /!\ with moltox21, some graphs are just one node, this specificity can't be taken into account with GraphiT, so we remove these graphs
    todel = []
    for i in range(len(moltox21_dataset_train)):
        if moltox21_dataset_train[i].edge_attr.shape[0] == 0:
            todel.append(i)
    mask = np.ones(len(moltox21_dataset_train), dtype=bool)
    mask[todel] = False
    moltox21_dataset_train = moltox21_dataset_train[torch.from_numpy(mask)]

    todel = []
    for i in range(len(moltox21_dataset_val)):
        if moltox21_dataset_val[i].edge_attr.shape[0] == 0:
            todel.append(i)
    mask = np.ones(len(moltox21_dataset_val), dtype=bool)
    mask[todel] = False
    moltox21_dataset_val = moltox21_dataset_val[torch.from_numpy(mask)]

    todel = []
    for i in range(len(moltox21_dataset_test)):
        if moltox21_dataset_test[i].edge_attr.shape[0] == 0:
            todel.append(i)
    mask = np.ones(len(moltox21_dataset_test), dtype=bool)
    mask[todel] = False
    moltox21_dataset_test = moltox21_dataset_test[torch.from_numpy(mask)]
    ...
    dataset = {
        'train': moltox21_dataset_train,
        'val': moltox21_dataset_val,
        'test': moltox21_dataset_test
        }
    ...
    net_params['n_classes'] = 12 # this should be called "num_tasks"
