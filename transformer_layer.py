...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, gamma, in_dim, out_dim, num_heads, double_attention, use_bias, adaptive_edge_PE, use_edge_features=False):
        super().__init__()
...
class GraphiT_GT_Layer(nn.Module):
    """
        Param: 
    """
    def __init__(self, gamma, in_dim, out_dim, num_heads, double_attention=False, dropout=0.0,
    ...
    def forward_p(self, p, e, k_RW=None, mask=None):
        '''
        Update positional encoding p
        '''
        p_in1 = p # for residual connection
    
        p = self.attention_p(p, e, k_RW=k_RW, mask=mask)  
    ...
    def forward(self, h, p, e, k_RW=None, mask=None):
        ...
        if self.update_pos_enc:
            p = self.forward_p(p, e, k_RW=k_RW, mask=mask)
        # p = self.forward_p(p, e, k_RW=k_RW, mask=mask)
